...
angular.module("RDash").controller("LoadDashBoard",
    function LoadDashBoard($scope,$rootScope,$http,$location) {
        ...
        //VERY DIRTY FIX TO SOLVE ANGULAR JS scope resolution
        //http://stackoverflow.com/questions/19310129/ng-model-no-longer-updates-after-typing-into-text-input
        ...
        $scope.reFormatData = function(){
            ...
            if($scope.direct){
                $scope.data['direct'] = $scope.direct.url;
            }
            if($scope.atom){
                //alert($scope.atom);
                $scope.data['atom'] = $scope.atom.url;
            }

            if($scope.rss){
                $scope.data['rss'] = $scope.rss.url;
            }
        ...
        $scope.DownloadIcalApi = function(){
			...
			var downloadLink = angular.element('<a></a>');
                        ...
                        downloadLink.attr('download', 'calendar.ics');
