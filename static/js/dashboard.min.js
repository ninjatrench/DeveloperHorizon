...
function LoadSession($scope,$rootScope,$http,$location){
    $rootScope.apiUrl = "http://localhost:5000/";
   ...
   $scope.removeError();
    ...
    $scope.saveiCal = function(){
        ...
        $http.get($rootScope.apiUrl+"download/"+$scope.sid)
            .success(function(response){
                $scope.toICAL = response;
                var blog = new Blob([$scope.toICAL], { type:"application/ical;charset=utf-8;" });
                var downloadLink = angular.element('<a></a>');
                downloadLink.attr('href',window.URL.createObjectURL(blob));
                downloadLink.attr('download', 'calendar.ical');
                downloadLink[0].click();
                setTimeout($scope.toogleICALAnime, 1000);
